...
/* 3 执行的命令写入reg_cmd寄存器 */
   ...
   /*********************   阻塞自己的时机  ***********************
      ...
      hd->my_channel->disk_done.sema_wait();
...
/* 4 检测硬盘状态是否可读 */
   ...
   hd->my_channel->lock.lock_release();
...
static void partition_scan(struct disk* hd, uint32_t ext_lba) {
   struct boot_sector* bs = (boot_sector*)sys_malloc(sizeof(struct boot_sector));
   ...
   while (part_idx++ < 4) {
	 ...
	 if (ext_lba == 0) {	 // 此时全是主分区
	    ...
	    partition_list.push_front(&hd->prim_parts[p_no].part_tag);
	    ...
	    partition_list.push_front(&hd->logic_parts[l_no].part_tag);
...
static bool partition_info(struct list_elem* pelem, int arg UNUSED) {
   ...
   printfk("   %s start_lba:%x, sec_cnt:%x\n",part->name, part->start_lba, part->sec_cnt);
...
void ide_init() {
   ...
   printfk("\n   all partition info\n");
   /* 打印所有分区信息 */
   partition_list.list_traversal(partition_info, (int)NULL);
