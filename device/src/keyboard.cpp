...
#define KBD_BUF_PORT 0x60	 // 键盘buffer寄存器端口号为0x60,这个 寄存器只有1B

/* 用转义字符定义部分控制字符 */
#define esc		'\033'	 // 八进制表示字符,也可以用十六进制'\x1b'
#define backspace	'\b'
#define tab		'\t'       //'\t'ASCII值为0x9
#define enter		'\n'   //'\r'的ASCII码值为0xd
...
/* 以通码make_code为索引的二维数组,即二维数组第i行表示通码为i的ASCII字符映射 */
...
/* 如果上次是以0xe0开头,将扫描码合并 */
   ...
   break_code = ((scancode & 0x0080) != 0);   // 判断是否为断码,如果低B最高位是1就说明是断码
   // 若是断码break_code(按键弹起时产生的扫描码)，则判断是不是控制键的断码，是的话就关闭控制状态
   if (break_code) {   

   /* 由于ctrl_r 和alt_r的make_code和break_code都是两字节,
   所以可用下面的方法取make_code,多字节的扫描码暂不处理 */
      uint16_t make_code = (scancode &= 0xff7f);   // 根据得到其make_code(按键按下时产生的扫描码)
    ...
    * 则将其加入到缓冲区kbd_buf中 */
      ...
      k_printf("unknown key\n");
   }
}

/* 键盘初始化 */
void keyboard_init() {
   k_printf("keyboard init start\n");
   //ioqueue_init(&kbd_buf);
   register_handler(0x21, (void*)intr_keyboard_handler);
   k_printf("keyboard init done\n");
}
