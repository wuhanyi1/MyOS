...
bool search_dir_entry(struct partition* part, struct dir* pdir, \
   ...
   if (all_blocks == nullptr) {
      printfk("search_dir_entry: sys_malloc for all_blocks failed");
      return false;
   }
...
bool sync_dir_entry(struct dir* parent_dir, struct dir_entry* p_de, void* io_buf) {
   ...
   while (block_idx < 140) {  // 文件(包括目录)最大支持12个直接块+128个间接块＝140个块
	 ...
	 if (block_lba == -1) {
	    printfk("alloc block bitmap for sync_dir_entry failed\n");
	    return false;
	 }
	 ...
	 if (block_idx < 12) {	    // 若是直接块
	    ...
	    if (block_lba == -1) {
	       ...
	       cur_part->block_bitmap.bitmap_set(block_bitmap_idx, 0);
...
bool delete_dir_entry(struct partition* part, struct dir* pdir, uint32_t inode_no, void* io_buf) {
   ...
   /* 在此扇区中找到目录项后,清除该目录项并判断是否回收扇区,随后退出循环直接返回 */
    ...
    /* 除目录第1个扇区外,若该扇区上只有该目录项自己,则将整个扇区回收 */
	 ...
	 part->block_bitmap.bitmap_set(block_bitmap_idx, 0);
...
int32_t dir_remove(struct dir* parent_dir, struct dir* child_dir) {
   ...
   if (io_buf == NULL) {
      printfk("dir_remove: malloc for io_buf failed\n");
      return -1;
   }
