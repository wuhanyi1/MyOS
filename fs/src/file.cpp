...
int32_t pcb_fd_install(int32_t globa_fd_idx) {
   ...
   if (local_fd_idx == MAX_FILES_OPEN_PER_PROC) {
      printfk("exceed max open files_per_proc\n");
      return -1;
   }
   return local_fd_idx;
}
...
int32_t inode_bitmap_alloc(struct partition* part) {
   int32_t bit_idx = part->inode_bitmap.bitmap_alloc(1);
   //int32_t bit_idx = bitmap_scan(&part->inode_bitmap, 1);
   if (bit_idx == -1) {
      return -1;
   }
   part->inode_bitmap.bitmap_set(bit_idx, 1);
...
int32_t block_bitmap_alloc(struct partition* part) {
   int32_t bit_idx = part->block_bitmap.bitmap_alloc(1);
   //int32_t bit_idx = bitmap_scan(&part->block_bitmap, 1);
   if (bit_idx == -1) {
      return -1;
   }
   part->block_bitmap.bitmap_set(bit_idx, 1);
...
int32_t file_create(struct dir* parent_dir, char* filename, uint8_t flag) {
   ...
   if (inode_no == -1) {
      printfk("in file_creat: allocate inode failed\n");
      return -1;
   }
...
/* 同步内存数据到硬盘 */
   ...
   cur_part->open_inodes.push_front(&new_file_inode->inode_tag);
...
int32_t file_write(struct file* file, const void* buf, uint32_t count) {
   ...
   if (io_buf == nullptr) {
      printfk("file_write: sys_malloc for io_buf failed\n");
      return -1;
   }
   uint32_t* all_blocks = (uint32_t*)sys_malloc(BLOCK_SIZE + 48);	  // 用来记录文件所有的块地址
   if (all_blocks == nullptr) {
      printfk("file_write: sys_malloc for all_blocks failed\n");
      return -1;
   }
   ...
   if (file->fd_inode->i_sectors[0] == 0) {
      ...
      if (block_lba == -1) {
	 printfk("file_write: block_bitmap_alloc failed\n");
	 return -1;
      }
 ...
 * 后面都统一在all_blocks中获取写入扇区地址 */
   ...
   /* 第一种情况:12个直接块够用*/
	 ...
	 while (block_idx < file_will_use_blocks) {
	    block_lba = block_bitmap_alloc(cur_part);
	    if (block_lba == -1) {
	       printfk("file_write: block_bitmap_alloc for situation 1 failed\n");
	       return -1;
	    }
	 ...
	 block_lba = block_bitmap_alloc(cur_part);
	 if (block_lba == -1) {
	    printfk("file_write: block_bitmap_alloc for situation 2 failed\n");
	    return -1;
	 }
	 ...
	 while (block_idx < file_will_use_blocks) {
	    block_lba = block_bitmap_alloc(cur_part);
	    if (block_lba == -1) {
	       printfk("file_write: block_bitmap_alloc for situation 3 failed\n");
	       return -1;
	    }
	    all_blocks[block_idx++] = block_lba;
   ...
   while (bytes_written < count) {      // 直到写完所有数据
      memset((void*)io_buf, 0, BLOCK_SIZE);
...
int32_t file_read(struct file* file, void* buf, uint32_t count) {
   ...
   uint8_t* io_buf = (uint8_t*)sys_malloc(BLOCK_SIZE);
   if (io_buf == nullptr) {
      printfk("file_read: sys_malloc for io_buf failed\n");
   }
   uint32_t* all_blocks = (uint32_t*)sys_malloc(BLOCK_SIZE + 48);	  // 用来记录文件所有的块地址
   if (all_blocks == nullptr) {
      printfk("file_read: sys_malloc for all_blocks failed\n");
      return -1;
   }
