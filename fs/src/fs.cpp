...
static bool mount_partition(struct list_elem* pelem, int arg) {
   ...
   if (!strcmp(part->name, part_name)) {
      ...
      if (cur_part->inode_bitmap.bits == nullptr) {
	    printfk("alloc memory failed!");
      }
...
int32_t sys_open(const char* pathname, uint8_t flags) {
  /* 对目录要用dir_open,这里只有open文件 */
   if (pathname[strlen(pathname) - 1] == '/') {
      printfk("can`t open a directory %s\n",pathname);
      return -1;
   }
   //ASSERT(flags <= 7);
   int32_t fd = -1;	   // 默认为找不到
   ...
   if (searched_record.file_type == FT_DIRECTORY) {
      printfk("can`t open a direcotry with open(), use opendir() to instead\n");
      dir_close(searched_record.parent_dir);
      return -1;
   }
   ...
   if (pathname_depth != path_searched_depth) {   // 说明并没有访问到全部的路径,某个中间目录是不存在的
      printfk("cannot access %s: Not a directory, subpath %s is`t exist\n", \
   ...
   if (!found && !(flags & O_CREAT)) {
      printfk("in path %s, file %s is`t exist\n", \
	    searched_record.searched_path, \
	    (strrchr(searched_record.searched_path, '/') + 1));
      dir_close(searched_record.parent_dir);
      return -1;
   } else if (found && flags & O_CREAT) {  // 若要创建的文件已存在
      printfk("%s has already exist!\n", pathname);
      dir_close(searched_record.parent_dir);
      return -1;
   }
...
int32_t sys_unlink(const char* pathname) {
   ...
   if (searched_record.file_type == FT_DIRECTORY) {
      printfk("can`t delete a direcotry with unlink(), use rmdir() to instead\n");
      dir_close(searched_record.parent_dir);
      return -1;
   }
   ...
   if (io_buf == NULL) {
      dir_close(searched_record.parent_dir);
      printfk("sys_unlink: malloc for io_buf failed\n");
      return -1;
   }
...
int32_t sys_mkdir(const char* pathname) {
   ...
   if (io_buf == NULL) {
      printfk("sys_mkdir: sys_malloc for io_buf failed\n");
      return -1;
   }
...
int32_t sys_rmdir(const char* pathname) {
   ...
   if (inode_no == -1) {
	 ...
	 if (!dir_is_empty(dir)) {	 // 非空目录不可删除
	    printfk("dir %s is not empty, it is not allowed to delete a nonempty directory!\n", pathname);
	 } else {
...
int32_t sys_chdir(const char* path) {
   ...
   if (inode_no != -1) {
	 ...
	 printfk("sys_chdir: %s is regular file or other!\n", path);
      }
   }
   dir_close(searched_record.parent_dir); 
   return ret;
...
void filesys_init() {
   ...
   /* 挂载分区 */
   partition_list.list_traversal(mount_partition, (int)default_part);
