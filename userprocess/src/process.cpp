...
/* 构建用户进程初始上下文信息，逐个函数主要是用来伪造中断栈 */
void start_process(void* filename_) {//
   void* function = filename_;
   task_struct* cur = running_thread();
   cur->self_kstack += sizeof(thread_stack);//此时self_kstack还是在线程栈的最底部
   intr_stack* proc_stack = (intr_stack*)cur->self_kstack;
   //开始伪造中断栈，因为将来要切换回用户态，所以CPU必认为有SS_old ESP_old	 
   proc_stack->edi = proc_stack->esi = proc_stack->ebp = proc_stack->esp_dummy = 0;
   proc_stack->ebx = proc_stack->edx = proc_stack->ecx = proc_stack->eax = 0;
   proc_stack->gs = 0;		 // 用户态用不上,直接初始为0
   proc_stack->ds = proc_stack->es = proc_stack->fs = SELECTOR_U_DATA;
   proc_stack->eip = (func*)function;	 // 待执行的用户程序地址
   proc_stack->cs = SELECTOR_U_CODE;
   proc_stack->eflags = (EFLAGS_IOPL_0 | EFLAGS_MBS | EFLAGS_IF_1);//伪造进程上去之后的eflags，IOPL为0，不能访问IO设备
   
   //首先要申请一个用户栈，然后设置用户栈的栈顶指针。注意用户栈我们统一地址为USER_STACK3_VADDR，所以这里要固定地址
   void* ustack = get_designed_vaddr(PF_USER,USER_STACK3_VADDR);
   if(!ustack){
       k_printf("user stack alloc failed\n");
   }
...
/* 击活页表,当进程切换到进程、进程切换到线程、线程切换到进程时，都需要更换地址资源，即页目录表 */
...
/* 若为内核线程,需要重新填充页表为0x100000 */
   ...
   if (p_thread->pgdir != nullptr)	{    // 用户态进程有自己的页目录表
      pagedir_phy_addr = addr_v2p((uint32_t)p_thread->pgdir);//将内核页目录表的虚拟地址转换为物理地址
   }
...
void process_activate(struct task_struct* p_thread) {
   ...
   /* 内核线程特权级本身就是0,处理器进入中断时并不会从tss中获取0特权级栈地址,不会访问TSS，故不需要更新esp0 */
   if (p_thread->pgdir) {
...
uint32_t* create_page_dir(void) {
   ...
   uint32_t* page_dir_vaddr = (uint32_t*)malloc_page(PF_KERNEL,1);
...
/************************** 1  先复制页表  *************************************/
   /*  page_dir_vaddr + 0x300*4 是内核页目录的第768项,将内核页目录表的768~1023这256个表项复制到用户页目录表 */
   memcpy((uint32_t*)((uint32_t)page_dir_vaddr + 0x300*4), (uint32_t*)(0xfffff000+0x300*4), 1024);
/*****************************************************************************/

/************************** 2  更新页目录地址 **********************************/
   uint32_t new_page_dir_phy_addr = addr_v2p((uint32_t)page_dir_vaddr);
   /* 页目录表地址是存入在页目录表的最后一项,更新页目录地址为新页目录的物理地址 */
